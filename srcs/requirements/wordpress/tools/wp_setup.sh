...
then
    ...
    wp config create --allow-root --dbname=$DB_NAME --dbuser=$ADMIN_LOGIN --dbpass=$ADMIN_PASSWORD --dbhost=mariadb --config-file=/var/www/wordpress/wp-config.php --skip-packages --skip-plugins 
